...
export class NotionConverter {
        ...
        return `<li>${this.richTextToHtml(
          ...
          bulletedListItem.children
            ? `<ul>${this.blocksToHtml(
                bulletedListItem.children,
                pageIdToSlugMap,
                true // Pass isInsideList = true for children
              )}</ul>`
            : ""
        }</li>`;
   ...
   * @param pageIdToSlugMap - A map of Notion page IDs to their corresponding slugs, used for resolving internal links.
   * @param isInsideList - Parameter to track if the conversion is inside a list (for nested lists)
   * @returns The HTML string representation of the blocks.
   */
  blocksToHtml(
    blocks: BlockWithChildren[],
    pageIdToSlugMap: Map<string, string>,
    isInsideList: boolean = false // Added parameter to track if inside a list
  ): string {
    ...
    for (let i = 0; i < blocks.length; i++) {
      ...
      // Prevent wrapping if already inside a list item's children
      const blockIsListItem =
        block.type === "bulleted_list_item" ||
        block.type === "numbered_list_item";

      if (block.type === "bulleted_list_item") {
        if (listType !== "ul" && !isInsideList) {
          if (listType) html += `</${listType}>`;
          html += "<ul>";
          listType = "ul";
        }
      } else if (block.type === "numbered_list_item") {
        if (listType !== "ol" && !isInsideList) {
          if (listType) html += `</${listType}>`;
          html += "<ol>";
          listType = "ol";
        }
      ...
      const currentBlockIsList = blockIsListItem;
