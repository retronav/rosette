...
const userObjectSchema = z
  .object({
    ...
    // but allowed by passthrough if present in the input data.
  })
  .passthrough();
 ...
 * Transforms an array of rich text objects into a single string.
 */
export const text = z // Covers 'rich_text' property type
  .object({
  ...
  .object({
    rollup: z.union([
      ...
      z.object({
        ...
        unsupported: z.object({}).passthrough(),
        function: z.string().optional()
      }),
      z.object({
        type: z.literal("incomplete"),
        incomplete: z.object({}).passthrough(),
        function: z.string().optional()
      })
    ])
  })
